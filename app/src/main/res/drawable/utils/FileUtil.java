...
public class FileUtil {
	...
	public static void saveUser(UserInfo user) {
		...
		try {
			oos = new ObjectOutputStream(AppUtils.getAppContext()
					.openFileOutput(com.huanchong.pet.utils.FileUtil.LOGIN, Context.MODE_PRIVATE));
			// 写入数据
	...
	public static void saveIsOk(int code) {
		...
		try {
			// 初始化
			oos = new ObjectOutputStream(AppUtils.getAppContext()
					.openFileOutput(com.huanchong.pet.utils.FileUtil.JPUSHISOK, Context.MODE_PRIVATE));
			// 写入数据
	...
	public static int getIsOk() {
		...
		try {
			// 获取对象
			ois = new ObjectInputStream(AppUtils.getAppContext().openFileInput(
					com.huanchong.pet.utils.FileUtil.JPUSHISOK));
			// 获取用户对象
